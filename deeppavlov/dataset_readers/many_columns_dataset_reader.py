...
class ManyColumnsDatasetReader(BasicClassificationDatasetReader):
    """
            ...
            if file.exists():
                ...
                if isinstance(x, list):
                    if isinstance(y, list):
                        data[data_type] = [([row[x_] for x_ in x], [row[y_] for y_ in y])
                                           for _, row in df.iterrows()]
                    else:
                        data[data_type] = [([row[x_] for x_ in x], str(row[y]).split(class_sep)) for _, row in
                                           df.iterrows()]
                else:
                    if isinstance(y, list):
