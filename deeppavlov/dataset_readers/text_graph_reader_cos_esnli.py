...
@register('text_graph_reader_cos_esnli')
class TextGraphReaderCosESNLI(DatasetReader):

    def read(self, data_path: str) -> Dict[str, List[Tuple[List[str], int]]]:

        data_path = expand_path(data_path)
        store_path = data_path / 'facts_store4.pickle'
        cos_path = data_path / 'cos_e.pickle'
        esnli_path = data_path / 'esnli.pickle'
        ...
        with open(cos_path, 'rb') as f:
            cos_store = pickle.load(f)
        with open(esnli_path, 'rb') as f:
            esnli_store = pickle.load(f)
        cos_data = list(cos_store.values())
        esnli_data = list(esnli_store.values())
        ...
        dataset = {"train": train_data+cos_data+esnli_data, "valid": valid_data, "test": test_data}
